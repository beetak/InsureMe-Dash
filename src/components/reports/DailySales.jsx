...
export default function DailySales() {
  ...
  }, [user, setUser]) // Added user and setUser to dependencies
  ...
  const handleSearch = async () => {
    ...
    try {
      const response = await InsuranceApi.get(
        `/product-payments/by-date?startDate=${formattedStartDate}&endDate=${formattedEndDate}`,
      )
  ...
  const generatePDF = () => {
    ...
    doc.text(410, 95, `Daily Sales Report`, { align: "right" })
