...
export default function QuotationModal({ setModal }) {
    ...
    const confirmPayment = useCallback(async(merchantReference)=>{
        try {
    ...
    const mobilePayment = useCallback(async (merchantRef, mobile, insuranceID, currency) => {
        ...
        try {
            ...
            if (result && result.data.token) {
                ...
                const mobilePaymentUrl = currency === "USD"? "/Mobilemoney/PayUSDEcocash": "/Mobilemoney/payEcocash"
                ...
                if(payResult.status === 200 && payResult.data.resultDescription ==="PENDING SUBSCRIBER VALIDATION"){
                    const res = await confirmPayment(payResult.data.merchantReference)
                    console.log(res)
                }
    ...
    const cashPayment = useCallback(async (quotation) => {
        ...
        const paymentBody = {
            ...
            insurerId: 1,
            amount: 0,
            duration: quotation.insuranceTerm,
        ...
        try {      
            const payResult = await InsuranceApi.post("/payments", paymentBody, { headers })
            setStatusMessage(`Payment successful for insurance ID: ${insuranceID}`)
                ...
                <div className={`flex-1 rounded-full shadow-lg border border-gray-300 overflow-hidden transition-all duration-300 ease-in-out ${showMobileInput ? '-ml-40' : ''}`}>
                    <div className="bg-gray-200 bg-gradient-to-b from-[#171b1b] to-[#4d97eb] opacity-70 backdrop-blur-3xl p-2">
                        <div className="flex text-xs text-white font-semibold text-center" style={{ lineHeight: '0.6' }}>
                            ...
                            {quotation.Licence && <div className="flex-1">Radio</div>}  
                        </div>
                    </div>
                    <div className="flex text-sm bg-gray-50 pt-1 text-gray-600 text-center" style={{ lineHeight: '0.6' }}>
                        ...
                        {quotation.Licence && <div className="flex-1">${quotation.Licence.RadioTVAmt}</div>}
                    </div>
                </div>
                {showMobileInput && (
