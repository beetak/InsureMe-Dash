...
const QuotationPrinter = ({ data }) => {
  const { vehicleData, userData } = useContext(StepperContext)
  ...
  const printCoverNote = useCallback(() => {
    ...
    addText(130, 240, `${vehicleData.make}\n${vehicleData.model}\n${vehicleData.YearManufacture}\n${vehicleData.VRN}`, 10, 'normal')

    // Notes
    addText(45, 280, 'Notes', 12, 'bold')
    addText(45, 290, `Vehicle Registration Number: ${vehicleData.VRN} is hereby insured against loss subject to the usual conditions of the \nFTP Cover until 31 December 2024 unless Notice of cancellation has been given`, 10, 'normal')
    ...
    setIsPrinting(false)
  }, [data, vehicleData, userData, formatPhoneNumber, getInsuranceType, formattedStampDuty, getDates])
