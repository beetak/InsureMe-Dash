...
export default function QuotationModal({ setModal }) {
    ...
    const mobilePayment = useCallback(async (quotation) => {
        ...
        try {
            ...
            if (authResult && authResult.data.token) {
                setPaymentToken(authResult.data.token)
                const paymentUrl = policyData.currency === "USD" ? "/Mobilemoney/PayUSDEcocash" : "/Mobilemoney/payEcocash"
                const payResult = await PaymentsApi.post(paymentUrl, {
                    customerMobileNumber: quotation.mobileNumber,
                    merchantRef: quotation.merchantRef,
    ...
    const cashPayment = useCallback(async (quotation) => {
        setProcessingPayment(true)
        try {
            const payResult = await InsuranceApi.post("/product-payments", {
                ...
                transactionDescription: vehicleData.VRN,
            ...
            if (payResult.data.code === "CREATED" && payResult.data.message === "Payment created") {
    ...
    }, [userId, vehicleData.VRN, userData.phoneNumber])

    const handlePaymentOption = useCallback((option, quotationId) => {
        setQuotations(prevQuotations => 
            prevQuotations.map(q => 
                q.quotationId === quotationId 
                    ? { ...q, paymentMethod: option, showMobileInput: option === 'mobile' } 
                    : { ...q, paymentMethod: '', showMobileInput: false }
            )
        )
    }, [])
    ...
    const QuotationItem = React.memo(({ quotation }) => {
        ...
        return (
            <div key={quotation.quotationId} className="flex flex-col mb-4">
                     ...
                     <div className="flex w-68 space-x-2 items-center relative">
                        <div className='relative'>
                            <button
                                ...
                                onClick={() => handlePaymentOption(quotation.paymentMethod ? '' : 'select', quotation.quotationId)}
                            >
                                {!quotation.paymentMethod && "Make Payment"}
                                {quotation.paymentMethod === "select" && "Select Payment"}
                                {quotation.paymentMethod === "cash" && "Cash Payment"}
                                {quotation.paymentMethod === "mobile" && "Mobile Payment"}
                            </button>
                            {quotation.paymentMethod === 'select' && (
                                <div className="absolute left-0 mt-1 w-36 z-10 bg-white border border-gray-300 rounded-md shadow-lg overflow-hidden text-sm text-center">
                                    <button
                                        className="w-full px-4 py-2 text-left hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                        onClick={() => handlePaymentOption('cash', quotation.quotationId)}
                                    >
                                        Cash Payment
                                    </button>
                                    <button
                                        className="w-full px-4 py-2 text-left hover:bg-gray-100 focus:outline-none focus:bg-gray-100"
                                        onClick={() => handlePaymentOption('mobile', quotation.quotationId)}
                                    >
                                        Mobile Payment
                                    </button>
                                </div>
                ...
                {quotation.paymentMethod && quotation.paymentMethod !== 'select' && (
                    <button
                        className="mt-2 py-1 px-3 rounded-full border border-gray-300 bg-green-500 text-white w-full disabled:opacity-50"
                        onClick={() => quotation.paymentMethod === 'cash' ? cashPayment(quotation) : mobilePayment(quotation)}
                    ...
                    >
                        {processingPayment ? (
                            ...
                            paymentStatus ? "Paid" : `Confirm ${quotation.paymentMethod.charAt(0).toUpperCase() + quotation.paymentMethod.slice(1)} Payment`
                        )}
                    </button>
                )}
            </div>
